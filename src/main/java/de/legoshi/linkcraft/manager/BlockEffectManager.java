...
public class BlockEffectManager {
    ...
    public boolean isEffectBlock(World world, int x, int y, int z) {
        ...
        // SELECT * FROM lc_effect_blocks WHERE world=? AND x=? AND y=? AND z=?;
        return result;
    }
    ...
    private BlockEffect getEffectData(String id, String type) {
        ...
        // Plan to do SELECT * FROM lc_type_effects (where type is the passed in type)
    ...
    private ArrayList<BlockEffect> getEffects(Block b) {
        ...
        // SELECT e.id, e.type FROM lc_effects e JOIN lc_effect_blocks eb ON e.set_id = eb.set_id
