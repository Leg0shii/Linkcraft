...
public interface CommonsUtils {
    ...
    static <T> boolean arrayContains(T[] array, T match) {
        return Arrays.stream(array).anyMatch((t) -> t == match);
    }
     ...
     */
    static String joinRepeatedString(String string, CharSequence charSequence, int repeat) {
        if (repeat <= 1) {
            return string;
        }
     ...
     * @return Collection joined as String
     */
    static <E> String joinIterable(String charSequence, Iterable<E> iterable) {
    ...
    static <E> String parenthesesString(Collection<E> collection) {
        return !isNullOrEmpty(collection) ? parenthesesString(joinIterable(collection)) : "";
    }
    ...
    static <T> String parenthesesString(T... objects) {
        return objects.length > 0 ? parenthesesString(Joiner.on(", ").join(objects)) : "";
    }
    ...
    static String middleSplit(String split, String string) {
        ...
        String[] splitter = {split.substring(0, mid), split.substring(mid)};
    ...
    static UUID formatUuid(String notFormat) {
        BigInteger decimal1 = new BigInteger(notFormat.substring(0, 16), 16);
        BigInteger decimal2 = new BigInteger(notFormat.substring(16, 32), 16);
     ...
     * @return SubLists.
     */
    static <E> List<List<E>> partitionList(List<E> list, int i) {
    ...
    static int parseInt(String toInt) {
        return !Strings.isNullOrEmpty(toInt) && regexNumber.matcher(toInt).matches() ? Integer.parseInt(toInt) : 0;
    }
    ...
    static boolean findAnnotation(List<? extends Annotation> annotations, Class<? extends Annotation> findAnnotation) {
